...
namespace UGF.EditorTools
{
    public class EditorToolbarExtension
    {
        ...
        static void Init()
        {
            ...
            var curPlatformIcon = Utility.Assembly.GetType("UnityEditor.Networking.PlayerConnection.ConnectionUIHelper")
                .GetMethod("GetIcon", BindingFlags.Static | BindingFlags.Public).Invoke(null, new object[] { EditorUserBuildSettings.activeBuildTarget.ToString() }) as GUIContent;
        ...
        static void ScanEditorToolClass()
        {
            ...
            var allEditorTool = editorDll.GetTypes().Where(tp =>
                (tp.IsClass && !tp.IsAbstract && tp.IsSubclassOf(typeof(EditorToolBase)) && tp.GetCustomAttribute(typeof(EditorToolMenuAttribute)) != null));
