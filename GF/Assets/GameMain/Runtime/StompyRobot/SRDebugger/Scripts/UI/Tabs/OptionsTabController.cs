...
namespace SRDebugger.UI.Tabs
{
    ...
    public class OptionsTabController : SRMonoBehaviourEx
    {
        ...
        protected void Populate()
        {
            ...
            var keyList = sortedOptions.Keys.ToList();
            keyList.Sort();
            for (var i=0;i<keyList.Count;i++)
            // foreach (var kv in sortedOptions)
            {
                var kv = new KeyValuePair<string, List<OptionDefinition>>(keyList[i], sortedOptions[keyList[i]]);
                if (kv.Value.Count == 0)
                {
                    continue;
                }
            ...
            Action<bool> SelectOpetion = (b) =>
            {
                if (b)
                {
                    foreach (var option in options)
                    {
                        ...
                        control.CachedTransform.gameObject.SetActive(categoryInstance.CategoryGroup.ChooseToggle.isOn);
                    }
                }
                ...
                foreach (var option in options)
                {
                    _options[option].CachedTransform.gameObject.SetActive(b);
                }
            };

            SelectOpetion(categoryInstance.CategoryGroup.ChooseToggle.isOn);
            
            categoryInstance.CategoryGroup.ChooseToggle.onValueChanged.AddListener((b) =>
            {
                SelectOpetion(b);
            });
        }
