...
public static class DataTableExtension
{
    ...
    public static void LoadDataTable(this DataTableComponent dataTableComponent, string dataTableName, string abTestGroupName, bool useBytes, object userData = null)
    {
        ...
        {
            var abTableFileName = Utility.Text.Format("{0}{1}{2}", dataTableName, SettingConst.AB_TEST_TAG, abTestGroupName);
            if (GF.Resource.HasAsset(AssetsPath.GetDataTablePath(abTableFileName, useBytes)) != GameFramework.Resource.HasAssetResult.NotExist)
            {
                tableFileName = abTableFileName;
            }
        }

        string assetName = AssetsPath.GetDataTablePath(tableFileName, useBytes);
    ...
    {
        string abTestGroup = GF.Setting.GetABTestGroup();
