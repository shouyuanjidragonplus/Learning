...
namespace Framework.Asset
{
    ...
    public class AssetPatchVersionConfig : ScriptableObject
    {
        ...
        static void PatchAllNormalAssets(int type)
        {
            ...
            {
                List<string> patchs = AssetConfigController.Instance.Groups[i].GetBundlePaths(true);
        ...
        static void _PatchSelectAssets(int type)
        {
            ...
            else
            {
                ...
                {
                    Debug.LogError("请选择Assets/Export目录下的资源");
                }
                else if (!Directory.Exists(select_path))
                {
                    Debug.LogError("请选择有效的文件夹目录");
                }
                else
                {
        ...
        static void PatchAsset(int type, string name)
        {
            ...
            Debug.Log(string.Format("asset : {0}, patch version : {1}, target : {2}", name, patchVersion.version, type == 0 ? "android" : "ios"));
        }
